...
export function createMediaAssetsRepository(sql: Sql): MediaAssetsRepository {
  return {
    ...
    async activeCount(clinicId) { const rows = await sql<[{ total: string }]>`SELECT COUNT(*)::text AS total FROM media_assets WHERE clinic_id = ${clinicId} AND deleted_at IS NULL`; return Number(rows[0]?.total ?? 0) },
    async createWithinQuota(data, limits) {
      return (sql.begin(async (tx) => {
        await tx`SELECT pg_advisory_xact_lock(hashtext(${data.clinicId}))`
        const usage = await tx<[{ total: string; bytes: string }]>`SELECT COUNT(*)::text AS total, COALESCE(SUM(byte_size), 0)::text AS bytes FROM media_assets WHERE clinic_id = ${data.clinicId} AND deleted_at IS NULL`
        ...
        const rows = await tx<MediaAsset[]>`INSERT INTO media_assets (clinic_id, uploaded_by, filename, content_type, byte_size, checksum, storage_key) VALUES (${data.clinicId}, ${data.uploadedBy ?? null}, ${data.filename}, ${data.contentType}, ${data.byteSize}, ${data.checksum}, ${data.storageKey}) RETURNING *`
        return rows[0]!
