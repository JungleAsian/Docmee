...
describe('sensitive retention repository', () => {
  it('purges raw transient payloads and scrubs AI usage metadata while preserving usage rows', async () => {
    ...
    expect(queries[5]).toContain('source_question_expires_at <= now()')
    expect(queries[6]).toContain('DELETE FROM knowledge_learning_events')
    expect(queries[7]).toContain('DELETE FROM knowledge_gaps')
    expect(queries[8]).toContain("status IN ('pending_review', 'rejected')")
