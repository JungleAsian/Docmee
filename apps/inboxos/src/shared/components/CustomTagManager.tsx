  ...
  return (
    <section className="rounded-[var(--crm-border-radius-md)] border border-[var(--crm-border-color)] bg-[var(--crm-card-bg)] p-3 shadow-[var(--crm-shadow-sm)]">
      <div className="flex items-center justify-between gap-2">
        <div>
          <h3 className="text-sm font-semibold">{t('tags.customTitle')}</h3>
          <p className="mt-0.5 text-[11px] text-[var(--crm-text-muted)]">{t('tags.customDescription')}</p>
        </div>
        {save.isSuccess && <span className="text-xs text-emerald-600">{t('common.saved')}</span>}
      </div>
      <div className="mt-3 space-y-2">
        {tags.map((tag, index) => (
          <div key={`${tag.name}-${index}`} className="flex items-center gap-2">
            ...
            />
            <input
              aria-label={t('tags.colorFor', { name: tag.name })}
              type="color"
              value={tag.color}
            ...
            <button type="button" aria-label={t('tags.moveUp', { name: tag.name })} disabled={index === 0} onClick={() => setTags((old) => {
            ...
            <button type="button" aria-label={t('tags.moveDown', { name: tag.name })} disabled={index === tags.length - 1} onClick={() => setTags((old) => {
              const next = [...old]; [next[index], next[index + 1]] = [next[index + 1], next[index]]; return next
            })} className="px-1 text-xs disabled:opacity-30">↓</button>
            <button type="button" aria-label={t('tags.delete', { name: tag.name })} onClick={() => setTags((old) => old.filter((_, itemIndex) => itemIndex !== index))} className="px-1 text-xs text-red-600">×</button>
          </div>
        ))}
      </div>
      <div className="mt-2 flex gap-2">
        <input value={newTag} onChange={(event) => setNewTag(event.target.value)} placeholder={t('tags.customPlaceholder')} className="min-w-0 flex-1 rounded border border-[var(--crm-border-color)] bg-transparent px-2 py-1 text-xs" />
        <button type="button" onClick={() => {
          const name = newTag.trim()
          if (name && !tags.some((tag) => tag.name.toLowerCase() === name.toLowerCase())) setTags((old) => [...old, { name, color: '#64748b', archived: false }])
          setNewTag('')
        }} className="rounded bg-[var(--crm-hover-bg)] px-2 py-1 text-xs font-semibold">{t('tags.add')}</button>
      </div>
      <button type="button" disabled={save.isPending} onClick={() => save.mutate()} className="mt-3 rounded bg-[var(--crm-primary-color)] px-3 py-2 text-xs font-bold text-white disabled:opacity-50">
        {save.isPending ? t('common.saving') : t('tags.save')}
      </button>
    </section>
  )
}
