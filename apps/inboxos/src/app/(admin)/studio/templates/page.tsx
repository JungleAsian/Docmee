...
const STARTER_TEMPLATES: StarterTemplate[] = [
  {
    ...
    language: 'es',
    body:
      'Hola {{1}}, tu cita en {{2}} esta confirmada para el {{3}} a las {{4}}. Responde CONFIRMAR para confirmar o CAMBIAR para pedir otro horario.',
  },
  {
    ...
    language: 'es',
    body:
      'Hola {{1}}, te recordamos tu cita en {{2}} manana a las {{3}}. Si no puedes asistir, responde CAMBIAR para ayudarte.',
  },
  {
    key: 'handoff_es',
    ...
    language: 'es',
    body:
      'Hola {{1}}, gracias por escribir a {{2}}. Un miembro del equipo revisara tu mensaje y te respondera pronto. Para emergencias, llama a servicios de emergencia.',
  },
  {
    ...
    language: 'es',
    body:
      'Hola {{1}}, gracias por visitar {{2}}. Si deseas compartir tu experiencia, puedes dejar una resena aqui: {{3}}. Responde STOP para no recibir estos mensajes.',
  },
  {
  ...
  return (
    <li className="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-gray-900">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {tpl.metaStatus && (
          <span className="rounded bg-blue-50 px-1.5 py-0.5 text-[10px] font-medium uppercase text-blue-700 dark:bg-blue-950/40 dark:text-blue-300">
            Meta {tpl.metaStatus}
          </span>
        )}
        {tpl.metaTemplateId && (
        ...
        <button
          ...
          onClick={onSync}
          disabled={syncBusy}
          className="rounded border border-gray-300 px-2 py-0.5 text-[10px] font-semibold uppercase text-gray-500 hover:bg-gray-50 disabled:opacity-50 dark:border-gray-700 dark:hover:bg-gray-800"
        >
          Sync Meta
        </button>
        <select
      ...
      {(tpl.metaLastSyncedAt || tpl.metaLastError) && (
        <p className={`mt-1.5 text-[11px] ${tpl.metaLastError ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>
          {tpl.metaLastError
            ? `Meta error: ${tpl.metaLastError}`
            : `Last synced: ${new Date(tpl.metaLastSyncedAt!).toLocaleString()}`}
        </p>
      )}
    </li>
  )
            ...
            {STARTER_TEMPLATES.map((template) => (
              ...
              >
                ...
                <span className="mt-1 block text-[11px] text-gray-500">
                  {t(`studio.templates.category.${template.category}` as const)} · {template.language}
                </span>
