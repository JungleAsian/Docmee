...
// Quick-replies manager — extracted from the standalone Quick Replies page so the
// same list / add / edit UI can also render as a section on the merged WhatsApp
// Templates page (item 9). `embedded` drops the page-level clinic switcher and
// offline banner (the host page owns those) and uses a section-sized heading.
// Everything is a single full-width column (1 reply per row).
...
export function QuickRepliesManager({ embedded = false }: { embedded?: boolean }) {
  const { t } = useI18n()
  ...
  return (
    <div className="space-y-4">
      {!embedded && !online && (
        <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-2.5 text-sm dark:border-amber-900 dark:bg-amber-950/40">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className={embedded ? 'text-base font-bold' : 'text-xl font-bold'}>{t('studio.quickReplies.title')}</h2>
      ...
      {!clinicId ? (
        ...
        <>
          ...
          <div className="clinic-card flex gap-3 bg-gray-50/60 p-3.5 dark:bg-gray-900/40">
...
function NewTemplateForm({ clinicId, onCreated }: { clinicId: string; onCreated: () => void }) {
  ...
  return (
    <form onSubmit={onSubmit} className="clinic-card space-y-2 p-4">
