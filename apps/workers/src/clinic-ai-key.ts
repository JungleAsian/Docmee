 ...
 * optionally, a specific model — J.zel's per-clinic model from
 * clinic.settings.aiAssistant; env/default fallback when undefined.
 */
...
/**
 * Embedder bound to the clinic's KB embed provider/model/key (clinic.settings
 * .aiAssistant.embedProvider/embedModel/baseURL). Local by default. Used by the
 * kb-embed worker (write path) and the patient bot's KB search (query path) so both
 * sides use the same provider — switching requires a KB re-index.
 */
export function resolveEmbedder(settings: unknown): (text: string) => Promise<number[]> {
  const ai =
    (settings as { aiAssistant?: { embedProvider?: string; embedModel?: string; baseURL?: string } } | null)
      ?.aiAssistant ?? {}
  const provider: EmbedProvider =
    ai.embedProvider === 'openai' || ai.embedProvider === 'gemini' || ai.embedProvider === 'custom'
      ? ai.embedProvider
      : 'local'
  const model = typeof ai.embedModel === 'string' ? ai.embedModel : undefined
  const baseURL = typeof ai.baseURL === 'string' ? ai.baseURL : undefined
  const apiKey = resolveClinicAiKey(settings, provider)
  return (text: string) => embed({ provider, text, apiKey, model, baseURL })
}
