...
const reviewSchema = z.object({ action: z.enum(['edit','reject','approve','rollback']), expectedRevision: z.number().int().positive(), content: z.string().trim().min(1).max(12000).optional(), staffConfirmed: z.boolean().optional(), historyId: z.string().uuid().optional() }).strict()
