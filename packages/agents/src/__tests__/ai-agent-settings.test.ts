...
describe('AI agent settings', () => {
  it('preserves clinic defaults for legacy nodes', () => {
    expect(resolveAiAgentSettings({}, { chatProvider: 'openai', model: 'clinic-model' })).toEqual({ provider: 'openai', model: 'clinic-model', maxTokens: 512 })
  })
  ...
  it.each(['openai', 'claude', 'custom', 'gemini'])('retains the inherited %s model', (provider) => {
    expect(resolveAiAgentSettings({ agentProvider: 'inherit', agentModel: '' }, { chatProvider: provider, model: 'clinic-model' }).model).toBe('clinic-model')
  })
  it('uses the service default after switching providers with a blank model', () => {
    expect(resolveAiAgentSettings({ agentProvider: 'claude', agentModel: '' }, { chatProvider: 'openai', model: 'clinic-model' })).toEqual({ provider: 'claude', model: '', maxTokens: 512 })
  })
